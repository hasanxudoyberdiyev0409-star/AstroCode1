...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Auth: connect modal forms to backend ---
    // use relative path so same origin (no CORS/port mismatch)
    const API_URL = '/api';

    function saveSession(token, user) {
        if (token) localStorage.setItem('token', token);
        if (user) localStorage.setItem('user', JSON.stringify({ name: user.name || '', email: (user.email||'').toLowerCase() }));
        renderHeaderProfile();
    }
    ...
    function generateAvatar(email, size=40) {
    ...
    function renderHeaderProfile() {
        ...
        if (user && user.email) {
            // show clickable avatar that navigates to profile
            navLoginBtn.innerHTML = `<img src="${generateAvatar(user.email,40)}" alt="avatar" class="nav-avatar" style="width:32px;height:32px;border-radius:50%;vertical-align:middle;margin-right:8px"> <span>${escapeHtml(user.name||user.email)}</span>`;
            navLoginBtn.classList.add('profile-btn');
            // ensure clicking avatar goes to profile page
            navLoginBtn.onclick = function(e){
                // if user clicks and holds modifier, preserve default
                if (e.metaKey||e.ctrlKey) return;
                window.location = '/profile.html';
            };
        } else {
            navLoginBtn.textContent = 'Kirish';
    ...
    // Initialize header based on stored session
    renderHeaderProfile();
});
...
// Note: older standalone login handler removed (we use modal handlers above).
// SIGNUP
...
async function login(username, password) {
  ...
  const data = await res.json();
  if (data.token) {
    localStorage.setItem("token", data.token); // JWT saqlanadi
